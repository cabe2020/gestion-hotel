...
export default function BookingsPage() {
  ...
  const load = useCallback(() => {
    fetch(`/api/bookings?page=${currentPage}&limit=${limit}`)
      .then((r) => r.json())
      .then((res) => {
  ...
  useEffect(() => {
    registerShortcutAction('newBooking', () => {
      setForm((f) => ({
        ...f,
        guestId: guests[0]?.id || '',
        roomId: rooms.filter((r) => r.status === 'available')[0]?.id || '',
      }));
      setShowModal(true);
    });
  ...
  const editAvailableRooms = rooms.filter(
    (r) => r.status === 'available' || (selectedBooking && r.id === selectedBooking.room?.id)
  );
  const upgradeAvailableRooms = rooms.filter(
    (r) => r.status === 'available' && (!selectedBooking || r.id !== selectedBooking.room?.id)
  );

  const editNights =
    editForm.checkIn && editForm.checkOut ? calcNights(editForm.checkIn, editForm.checkOut) : 0;
    ...
    <div>
      ...
      <div className="p-6 space-y-6">
                  ...
                  return (
                    <tr key={booking.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm">
                        <span
                          className={
                            pending > 0 ? 'text-red-600 font-medium' : 'text-green-600 font-medium'
                          }
                        >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Autocomplete
                  ...
                  displayValue={
                    form.guestId
                      ? guests.find((g) => g.id === form.guestId)?.firstName +
                          ' ' +
                          guests.find((g) => g.id === form.guestId)?.lastName || ''
                      : ''
                  }
                  onSelect={(item) => {
                ...
                >
                  ...
                  {availableRooms.map((r) => (
                    <option key={r.id} value={r.id}>
                      Hab. {r.number} - {r.roomType.name} (${r.roomType.basePrice}/noche)
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  value={form.adults}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, adults: parseInt(e.target.value) || 1 }))
                  }
                  className="input-field"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  value={form.children}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, children: parseInt(e.target.value) || 0 }))
                  }
                  className="input-field"
                />
              </div>
                ...
                return (
                  <div className="bg-amber-50 rounded-lg p-3">
                    <p className="text-sm text-amber-800">
                      Tarifa base: {formatCurrency(base)}/noche
                    </p>
                    <p className="text-sm text-amber-800">
                      + Suplemento {form.adults - 1} persona{form.adults - 1 > 1 ? 's' : ''}:{' '}
                      {formatCurrency(surcharge)}/noche
                    </p>
                    <p className="text-sm font-medium text-amber-900">
                      = {formatCurrency(total)}/noche
                    </p>
                  </div>
                );
              })()}
        ...
        <Modal
          ...
          title={`Editar Reserva - ${selectedBooking?.code || ''}`}
          size="lg"
        >
          <form onSubmit={handleEditSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Autocomplete
                  ...
                  displayValue={
                    editForm.guestId
                      ? guests.find((g) => g.id === editForm.guestId)?.firstName +
                          ' ' +
                          guests.find((g) => g.id === editForm.guestId)?.lastName || ''
                      : ''
                  }
                  onSelect={(item) => {
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="text-sm text-gray-600">
                Noches: <span className="font-medium text-gray-900">{editNights}</span>
              </p>
                ...
                <>
                  <p className="text-sm text-gray-600 mt-1">
                    ...
                    <span className="font-medium text-gray-900">
                      {formatCurrency(editBaseRate)}/noche
                    </span>
                  </p>
                  <p className="text-sm text-amber-700 mt-1">
                    + Suplemento {editForm.adults - 1} persona{editForm.adults - 1 > 1 ? 's' : ''}:{' '}
                    <span className="font-medium">{formatCurrency(editSurcharge)}/noche</span>
                  </p>
                  <p className="text-sm text-gray-600 mt-1">
                    Tarifa con ocupacion:{' '}
                    <span className="font-medium text-gray-900">
                      {formatCurrency(editRate)}/noche
                    </span>
                  </p>
                </>
              ) : (
              ...
              <p className="text-sm text-gray-600 mt-1">
                Total:{' '}
                <span className="font-medium text-gray-900">
                  {formatCurrency(editRate * editNights)}
                </span>
              </p>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm((f) => ({ ...f, adults: parseInt(e.target.value) || 1 }))
                  }
                  className="input-field"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm((f) => ({ ...f, children: parseInt(e.target.value) || 0 }))
                  }
                  className="input-field"
                />
              </div>
        ...
        <Modal
          ...
          title={`Cambiar Habitación - ${selectedBooking?.code || ''}`}
        >
          {selectedBooking && (
            <form onSubmit={handleUpgradeSubmit} className="space-y-4">
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Tarifa actual:{' '}
                  <span className="font-medium text-gray-900">
                    {formatCurrency(selectedBooking.roomRate)}/noche
                  </span>
                </p>
              </div>
              <div>
                  ...
                  return (
                    <div className="bg-blue-50 rounded-lg p-4">
                      <p className="text-sm text-blue-800">
                        Nueva tarifa:{' '}
                        <span className="font-medium">{formatCurrency(newRate)}/noche</span>
                      </p>
                      <p className="text-sm text-blue-800 mt-1">
                        Nuevo total:{' '}
                        <span className="font-medium">
                          {formatCurrency(newRate * selectedBooking.totalNights)}
                        </span>
                      </p>
                    </div>
                  );
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" className="btn-primary flex-1" disabled={!upgradeRoomId}>
                  Cambiar Habitación
                </button>
                <button
                  type="button"
        ...
        <Modal
          ...
          title={`Cobrar - ${selectedBooking?.code || ''}`}
        >
          {selectedBooking && (
        ...
        <Modal
          isOpen={showUpsells}
          onClose={() => setShowUpsells(false)}
          title={`Upsells - ${selectedBooking?.code || ''}`}
          size="lg"
        >
          {selectedBooking && (
            <div className="space-y-4">
