...
export async function POST(request: Request) {
  try {
    ...
    if (!roomType || roomType.hotelId !== hotelId) {
      return NextResponse.json({ error: 'Tipo de habitacion no encontrado' }, { status: 404 });
    }
    ...
    const booking = await prisma.$transaction(async (tx) => {
      ...
      const applicableRate = roomType.ratePlans.find((rp) => totalNights >= rp.minStay);
      ...
      const occupancyNote =
        occupancySurcharge > 0
          ...
          : '';
      const taxNote =
        hotel.taxRate > 0 ? ` | Impuesto (${hotel.taxRate}%): $${taxAmount.toFixed(2)}` : '';
