...
const emptyRateForm = { name: "", roomTypeId: "", startDate: "", endDate: "", price: 0, minStay: 1 };
...
export default function SettingsPage() {
  ...
  const openEditType = (rt: RoomTypeData) => {
    ...
    setTypeForm({ name: rt.name, code: rt.code, basePrice: rt.basePrice, capacity: rt.capacity, amenities: rt.amenities });
    setShowTypeModal(true);
  };
  ...
  const openEditUpsell = (us: UpsellData) => {
    ...
    setUpsellForm({ name: us.name, description: us.description, price: us.price, category: us.category, active: us.active });
    setShowUpsellModal(true);
  };
        ...
        <form onSubmit={handleSaveHotel} className="card space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={hotel.name} onChange={e => setHotel({ ...hotel, name: e.target.value })} className="input-field" required />
            </div>
            <div>
              <label className="label-field">Email</label>
              <input type="email" value={hotel.email} onChange={e => setHotel({ ...hotel, email: e.target.value })} className="input-field" />
            </div>
            <div>
              <label className="label-field">Telefono</label>
              <input type="text" value={hotel.phone} onChange={e => setHotel({ ...hotel, phone: e.target.value })} className="input-field" />
            </div>
            <div>
              <label className="label-field">Moneda</label>
              <select value={hotel.currency} onChange={e => setHotel({ ...hotel, currency: e.target.value })} className="input-field">
      ...
      Guardar
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            {roomTypes.map(rt => (
              <div key={rt.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  <p className="text-sm font-semibold text-gray-900">{rt.name} ({rt.code})</p>
                  <p className="text-xs text-gray-500">Precio base: ${rt.basePrice}/noche - Capacidad: {rt.capacity}</p>
                  {rt.amenities && <p className="text-xs text-gray-400 mt-1">{rt.amenities}</p>}
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => openEditType(rt)} className="p-1.5 rounded hover:bg-blue-50 text-blue-600" title="Editar">
                    <Pencil className="h-4 w-4" />
                  </button>
                  <button onClick={() => handleDeleteRoomType(rt.id)} className="p-1.5 rounded hover:bg-red-50 text-red-600" title="Eliminar">
                    <Trash2 className="h-4 w-4" />
                  </button>
                </div>
              </div>
            ))}
            {roomTypes.length === 0 && <p className="text-sm text-gray-500">No hay tipos de habitacion</p>}
          </div>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Tipo Hab.</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Desde</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Hasta</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Precio/Noche</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Min. Noches</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {ratePlans.map(rp => (
                    <tr key={rp.id} className={`hover:bg-gray-50 ${isRateActive(rp) ? "bg-green-50" : ""}`}>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700">{formatDate(rp.startDate)}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{formatDate(rp.endDate)}</td>
                      <td className="px-4 py-3 text-sm font-medium text-gray-900">{formatCurrency(rp.price)}</td>
                      <td className="px-4 py-3 text-sm text-gray-700">{rp.minStay}</td>
                      <td className="px-4 py-3">
                        {isRateActive(rp) ? (
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Activa</span>
                        ) : (
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">Inactiva</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEditRate(rp)} className="p-1.5 rounded hover:bg-blue-50 text-blue-600" title="Editar">
                            <Pencil className="h-4 w-4" />
                          </button>
                          <button onClick={() => handleDeleteRatePlan(rp.id)} className="p-1.5 rounded hover:bg-red-50 text-red-600" title="Eliminar">
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </div>
                      </td>
        ...
        <Modal
          ...
          onClose={() => { setShowTypeModal(false); setEditingTypeId(null); }}
        ...
        >
          <form onSubmit={handleSaveRoomType} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="label-field">Nombre</label>
                <input type="text" value={typeForm.name} onChange={e => setTypeForm(f => ({ ...f, name: e.target.value }))} className="input-field" required />
              </div>
              <div>
                <label className="label-field">Codigo</label>
                <input type="text" value={typeForm.code} onChange={e => setTypeForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} className="input-field" required />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="label-field">Precio Base</label>
                <input type="number" step="0.01" value={typeForm.basePrice} onChange={e => setTypeForm(f => ({ ...f, basePrice: parseFloat(e.target.value) || 0 }))} className="input-field" required />
              </div>
              <div>
                <label className="label-field">Capacidad</label>
                <input type="number" value={typeForm.capacity} onChange={e => setTypeForm(f => ({ ...f, capacity: parseInt(e.target.value) || 1 }))} className="input-field" required />
              </div>
            </div>
            <div>
              <label className="label-field">Amenidades (separadas por coma)</label>
              <input type="text" value={typeForm.amenities} onChange={e => setTypeForm(f => ({ ...f, amenities: e.target.value }))} className="input-field" placeholder="WiFi, TV, A/C, Minibar" />
            </div>
            <div className="flex gap-3 pt-2">
              <button type="submit" className="btn-primary flex-1">{editingTypeId ? "Guardar" : "Crear"}</button>
              <button type="button" onClick={() => { setShowTypeModal(false); setEditingTypeId(null); }} className="btn-secondary flex-1">Cancelar</button>
            </div>
          </form>
        </Modal>
        ...
        <Modal
          ...
          onClose={() => { setShowRateModal(false); setEditingRateId(null); }}
          title={editingRateId ? "Editar Tarifa" : "Nueva Tarifa"}
        >
          <form onSubmit={handleSaveRatePlan} className="space-y-4">
            <div>
              <label className="label-field">Nombre</label>
              <input type="text" value={rateForm.name} onChange={e => setRateForm(f => ({ ...f, name: e.target.value }))} className="input-field" placeholder="Temporada Alta, Semana Santa..." required />
            </div>
            <div>
              <label className="label-field">Tipo de Habitacion</label>
              <select value={rateForm.roomTypeId} onChange={e => setRateForm(f => ({ ...f, roomTypeId: e.target.value }))} className="input-field" required>
                <option value="">Seleccionar</option>
                {roomTypes.map(rt => <option key={rt.id} value={rt.id}>{rt.name} (${rt.basePrice}/noche base)</option>)}
              </select>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="label-field">Fecha Inicio</label>
                <input type="date" value={rateForm.startDate} onChange={e => setRateForm(f => ({ ...f, startDate: e.target.value }))} className="input-field" required />
              </div>
              <div>
                <label className="label-field">Fecha Fin</label>
                <input type="date" value={rateForm.endDate} onChange={e => setRateForm(f => ({ ...f, endDate: e.target.value }))} className="input-field" required />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="label-field">Precio por Noche</label>
                <input type="number" step="0.01" value={rateForm.price} onChange={e => setRateForm(f => ({ ...f, price: parseFloat(e.target.value) || 0 }))} className="input-field" required />
              </div>
              <div>
                <label className="label-field">Minimo de Noches</label>
                <input type="number" min={1} value={rateForm.minStay} onChange={e => setRateForm(f => ({ ...f, minStay: parseInt(e.target.value) || 1 }))} className="input-field" />
              </div>
            </div>
            <div className="flex gap-3 pt-2">
              <button type="submit" className="btn-primary flex-1">{editingRateId ? "Guardar" : "Crear"}</button>
              <button type="button" onClick={() => { setShowRateModal(false); setEditingRateId(null); }} className="btn-secondary flex-1">Cancelar</button>
            </div>
          </form>
        </Modal>
      </div>
