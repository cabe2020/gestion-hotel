...
export default function InvoicesPage() {
  ...
  const [hotelInfo, setHotelInfo] = useState<{ name: string; address: string; email: string; phone: string } | null>(null);
  ...
  const load = () => {
    ...
    fetch("/api/bookings").then(r => r.json()).then(res => { const b = Array.isArray(res) ? res : res.data; setBookings(b.filter((bk: Booking) => !bk.invoice)); });
    fetch("/api/hotels").then(r => r.json()).then(h => {
      ...
      <div className="p-6 space-y-6">
        ...
        {invoices.length === 0 ? (
          <div className="card text-center py-12">
            ...
            <p className="text-xs text-gray-400 mt-1">Crea facturas desde reservas que no tengan factura asignada</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Huesped</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Habitacion</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {invoices.map(inv => (
                  <tr key={inv.id} className={`hover:bg-gray-50 ${inv.cancelled ? "opacity-60" : ""}`}>
                    <td className="px-4 py-3 text-sm font-mono font-medium text-blue-600">{inv.number}</td>
                    <td className="px-4 py-3 text-sm text-gray-700">{formatDate(inv.date)}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{inv.booking?.guest?.firstName} {inv.booking?.guest?.lastName}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        {userRole === "admin" && !inv.cancelled && (
                          <button onClick={() => handleCancelClick(inv.id)} className="p-1.5 rounded hover:bg-red-50 text-red-600" title="Anular factura">
        ...
        <Modal isOpen={showCreate} onClose={() => { setShowCreate(false); setSelectedBooking(""); }} title="Generar Factura">
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              ...
              <select value={selectedBooking} onChange={e => setSelectedBooking(e.target.value)} className="input-field" required>
                ...
                {bookings.map(b => (
                  <option key={b.id} value={b.id}>{b.code} - {b.guest?.firstName} {b.guest?.lastName} - Hab. {b.room?.number} - {formatCurrency(b.totalAmount)}</option>
                ))}
              </select>
            </div>
            {selectedBooking && (() => {
              ...
              return (
                <div className="bg-gray-50 rounded-lg p-4 text-sm space-y-1">
                  ...
                  <p>Noches: {b.totalNights} x {formatCurrency(b.roomRate)}/noche</p>
                  <p className="font-semibold">Total: {formatCurrency(b.totalAmount)} | Pagado: {formatCurrency(b.paidAmount)}</p>
                </div>
              );
            })()}
                ...
                <div className="bg-gray-50 rounded-lg p-3 text-sm space-y-1">
                  {folioItemsForBooking.map(fi => (
                    <div key={fi.id} className="flex justify-between">
                      <span>{fi.concept} <span className="text-gray-400">({categoryLabels[fi.category] || fi.category})</span></span>
            ...
            <div className="flex gap-3 pt-2">
              <button type="submit" className="btn-primary flex-1">Generar Factura</button>
              <button type="button" onClick={() => { setShowCreate(false); setSelectedBooking(""); }} className="btn-secondary flex-1">Cancelar</button>
            </div>
          </form>
        </Modal>

        <Modal isOpen={showPrint} onClose={() => setShowPrint(false)} title="Vista de Impresion" size="lg">
          {selectedInvoice && (() => {
            ...
            return (
              <div id="invoice-print" className="space-y-6">
                <div className="flex justify-between items-start border-b-2 border-gray-900 pb-4">
                  <div>
                    ...
                    {inv.cancelled && inv.cancelReason && (
                      <p className="text-sm text-red-600 font-medium mt-1">Motivo: {inv.cancelReason}</p>
                    )}
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold text-gray-900">{hotelInfo?.name || "Hotel"}</p>
                  ...
                  <tbody>
                    {folioItems.length > 0 ? (
                      ...
                      <tr className="border-b border-gray-100">
                        ...
                        <td className="text-right py-2 font-medium">{formatCurrency(b?.totalAmount || 0)}</td>
                      </tr>
                    )}
                  </tbody>
                  <tfoot>
                    <tr className="border-t border-gray-300">
                      ...
                      <td className="text-right py-2">{formatCurrency(inv.total - inv.taxAmount)}</td>
                    </tr>
                    <tr>
                    ...
                    <tr className="border-t-2 border-gray-900">
                      ...
                      <td className="text-right py-2 font-bold text-lg">{formatCurrency(inv.total)}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td className="text-right py-2">{formatCurrency(inv.total - (b?.paidAmount || 0))}</td>
                    </tr>
                  </tfoot>
                </table>
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button onClick={() => setShowPrint(false)} className="btn-secondary flex-1">Cerrar</button>
                </div>
              </div>
            );
          })()}
        ...
        <Modal isOpen={showCancel} onClose={() => setShowCancel(false)} title="Anular Factura">
          <div className="space-y-4">
            <p className="text-sm text-red-600 font-medium">Esta accion no se puede deshacer. La factura sera marcada como anulada.</p>
            <div>
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={handleCancelConfirm} className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 flex-1" disabled={!cancelReason.trim()}>
                <Ban className="h-4 w-4 inline mr-1" /> Anular Factura
              </button>
              <button onClick={() => setShowCancel(false)} className="btn-secondary flex-1">Cancelar</button>
            </div>
          </div>
        </Modal>
      </div>
