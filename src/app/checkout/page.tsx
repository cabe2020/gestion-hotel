      ...
      <div className={`card ${urgent ? "border-red-200" : ""}`}>
        ...
        <div className="space-y-1 mb-4">
          ...
          <div className="flex items-center gap-2 mt-2">
            <span className="text-sm text-gray-600">Total: {formatCurrency(booking.totalAmount)}</span>
            <span className="text-sm text-gray-600">Pagado: {formatCurrency(booking.paidAmount)}</span>
            {pending > 0 && <span className="text-sm font-medium text-red-600">Pendiente: {formatCurrency(pending)}</span>}
            {pending <= 0 && <span className="text-sm font-medium text-green-600">Pagado completo</span>}
          </div>
        </div>
        <button
      ...
      <div className="p-6 space-y-6">
        ...
        {lateCheckouts.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {lateCheckouts.map(b => <CheckoutCard key={b.id} booking={b} urgent />)}
            </div>
          </div>
        )}
        ...
        <div>
          ...
          {todayCheckouts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {todayCheckouts.map(b => <CheckoutCard key={b.id} booking={b} />)}
            </div>
          )}
        </div>

        <div>
          <h2 className="text-lg font-semibold text-gray-900 mb-3">Todos los Huéspedes Actualmente</h2>
            ...
            {checkedIn.map(b => (
              <div key={b.id} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Hab. {b.room?.number} - Salida: {formatDate(b.checkOut)}</p>
                </div>
                <button onClick={() => handleCheckOut(b.id)} className="btn-primary text-xs py-1 px-3">
                  Check-out
                </button>
              </div>
            ))}
            {checkedIn.length === 0 && <p className="text-sm text-gray-500">No hay huéspedes en el hotel</p>}
          </div>
        </div>
      </div>
    </div>
