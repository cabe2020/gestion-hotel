...
export function generateOccupancyReportPDF(
  data: { date: string; totalRooms: number; occupiedRooms: number; occupancyRate: number; avgRate: number; revenue: number }[],
  ...
  const avgOccupancy = data.length > 0 ? Math.round(data.reduce((s, d) => s + d.occupancyRate, 0) / data.length) : 0;
  const maxOccupancy = data.length > 0 ? Math.max(...data.map((d) => d.occupancyRate)) : 0;
  const totalRevenue = data.reduce((s, d) => s + d.revenue, 0);
  const avgRate = data.length > 0 ? Math.round((data.reduce((s, d) => s + d.avgRate, 0) / data.length) * 100) / 100 : 0;
...
export function generateRevenueReportPDF(
  data: { date: string; roomRevenue: number; folioRevenue: number; totalRevenue: number; payments: number }[],
  hotel: { name: string; currency: string }
): jsPDF {
  const doc = new jsPDF();
  const currency = hotel.currency || "USD";
